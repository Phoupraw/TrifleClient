    ...
    private static YetAnotherConfigLib toYACL(ConfigClassHandler<TCConfigs> config) {
        ...
        return YetAnotherConfigLib.createBuilder()
          ...
          .category(ConfigCategory.createBuilder()
            ...
            .option(Option.<Boolean>createBuilder()
              .name(Text.of("自动收割"))
              .description(OptionDescription.of(Text.of("自动收割长成的甘蔗和作物，并补种。")))
              .binding(of(defaults::isAutoHarvest, instance::isAutoHarvest, instance::setAutoHarvest))
              .controller(TickBoxControllerBuilder::create)
              .build())
            .option(Option.<Boolean>createBuilder()
              .name(Text.of("持续举盾"))
              .description(OptionDescription.of(Text.of("当玩家副手持盾且不在使用主手物品时，自动持续举盾。")))
              .binding(of(defaults::isAlwaysShield, instance::isAlwaysShield, instance::setAlwaysShield))
              .controller(TickBoxControllerBuilder::create)
              .build())
            .option(Option.<Boolean>createBuilder()
              .name(Text.of("自动选中工具"))
              .description(OptionDescription.of(Text.of("玩家开始挖掘方块时自动选中快捷栏中合适的工具，停止挖掘后选中之前的物品。")))
              .binding(of(defaults::isAutoSwitchTool, instance::isAutoSwitchTool, instance::setAutoSwitchTool))
              .controller(TickBoxControllerBuilder::create)
              .build())
            .option(Option.<Boolean>createBuilder()
              .name(Text.of("水上漂"))
              .description(OptionDescription.of(Text.of("让玩家在任何流体表面行走，只要玩家当前不处于流体中且不在潜行。")))
              .binding(of(defaults::isWalkOnFluid, instance::isWalkOnFluid, instance::setWalkOnFluid))
              .controller(TickBoxControllerBuilder::create)
              .build())
            .option(Option.<Integer>createBuilder()
              ...
              .build())
            .option(Option.<Integer>createBuilder()
              .name(Text.of("聊天文本最大长度"))
              .description(OptionDescription.of(Text.of("用于执行长客户端指令。向服务端发送长于256的消息会被踢出！")))
              .binding(of(defaults::getChatMaxLen, instance::getChatMaxLen, instance::setChatMaxLen))
              .controller(option -> IntegerFieldControllerBuilder.create(option)
                .range(256, Integer.MAX_VALUE))
              .build())
            .option(Option.<Double>createBuilder()
              .name(Text.of("限制最大移速"))
              .description(OptionDescription.of(Text.of("防止过多属性加成使移动速度过大")))
              .binding(of(defaults::getLimitSpeed, instance::getLimitSpeed, instance::setLimitSpeed))
              .controller(option -> DoubleSliderControllerBuilder.create(option)
                .range(0.0, 0.5)
                .step(0.005))
              .build())
            .option(Option.<Float>createBuilder()
              .name(Text.of("限制最大跳跃速度"))
              .description(OptionDescription.of(Text.of("防止过多属性加成使跳跃速度过大")))
              .binding(of(defaults::getLimitJump, instance::getLimitJump, instance::setLimitJump))
              .controller(option -> FloatSliderControllerBuilder.create(option)
                .range(0.4f, 1f)
                .step(0.01f)
                .formatValue(v -> Text.of("%.2f".formatted(v))))
              .build())
            .option(Option.<Float>createBuilder()
              .name(Text.of("环境亮度"))
