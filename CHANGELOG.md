# 0.1.0
## 新增
- 由屏幕中心指向世界中坐标的HUD的API。
- 搜索方块的指令`/trifleclient find block [block]`
# 0.2.0
## 新增
- 高亮方块区域边框的API。
## 更改
- 搜索到的方块在移除后会自动开始新一轮搜索。
- 搜索到的方块会高亮边框。
# 0.2.1
## 修复
- 搜索到的方块被移除后会找不到下一个方块。
# 0.3.0
## 新增
- 挖掘延迟。
# 0.4.0
## 新增
- 自动攻击
- 快速移动
- 常态加速
- 自动暴击
- 防止摔落伤害
- 上台阶高度最小为1.375
## 修复
- 退出游戏时有可能抛出异常。
# 0.4.1
## 新增
- 更详细的编解码异常日志
# 0.4.2
## 修复
- 未给`at`设置`remap=false`
# 0.4.3
## 修复
- 与《Eclipse's Tweakeroo》冲突
# 0.4.4
## 更改
- 潜行时不再向服务端发OnGround包
- 快速移动改为少量多段
## 修复
- 快速移动上楼梯会显著减速
- 快速移动下楼梯难以触发快速下楼梯
# 0.5.0
## 新增
- 所有维度的`ambientLight`至少为`0.08`
# 0.6.0
## 新增
- 距离玩家24格以内的物品实体和经验球会发光，颜色为白色到金色之间不断渐变。
- 手持繁殖物品按住shift双击右键可以一键繁殖周围所有生物
# 0.6.1
## 更改
- 飞行时也会不断发送OnGround信包了。
- 倍速移动改为额外移动，以便触发基于移动的探测。
# 0.6.2
## 更改
- 物品和经验球发光范围改为以相机为中心。
# 0.6.3
## 修复
- 时间过大导致浮点数溢出到无穷，使得发光效果不渐变。
# 0.6.4
## 修复
- 发光渐变代码错误。
# 0.7.0
## 新增
- 指令`/trifleclient spruce [pos]`，可以在指定位置的2×2灰化土上用主手上的云杉树苗持续种植，以便AFK大型云杉树场。
# 0.7.1
## 更改
- 更改方块高亮图层，试图渲染在格雷矿物图层之上。
# 0.8.0
## 新增
- 右键聊天栏里的文件链接会打开其所在的文件。
# 0.9.0
## 新增
- 《通用机械》的原子分解机也算作杀戮光环的武器。
- 按住指定快捷键时只能破坏与首次破坏相同的方块。
# 0.10.0
## 新增
- 《通用机械》的Meka工具也算作杀戮光环的武器。
# 0.11.0
## 新增
- 检测到《通用机械》时会在日志打印一条消息。
- 指令`/trifleclient strip`，可以在手持斧头时持续给周围原木去皮。
## 更改
- Logger的名称从`trifleclient`改为`TrifleClient`。
- 前移方块高亮渲染时机。
# 0.12.0
## 更改
- 物品和经验球发光的范围会随“实体渲染距离”而缩放。
- 鞘翅飞行时不发送`OnGround`信包。
- 指令`/trifleclient spruce <pos>`改为`/trifleclient spruce`，位置为准心对着的方块。
# 0.12.1
## 修复
- `/trifleclient spruce`选取了高一格的方块。
# 0.13.0
## 新增
- 可以执行`/trifleclient crit`开关自动暴击（无法保存，每次启动游戏时重设为启用）。
# 0.14.0
## 新增
- 更详细的信包报错。
## 修复
- `/trifleclient crit`的消息颠倒。
# 0.15.0
## 新增
- 穿着鞘翅时忽略服务端同步飞行能力。（默认关闭）
- 穿着鞘翅时可以如创造般飞行。
# 0.15.1
## 修复
- 有时在结束鞘翅飞行时会突然向上看。
# 0.16.0
## 新增
- 把《模组检查》向服务器发送的模组列表改为空列表。
## 修复
- 有时在结束鞘翅创造飞行时会突然向上看。
# 0.16.1
## 修复
- 鞘翅创造飞行缓降错误。
# 0.17.0
## 更改
- 禁用鞘翅创造飞行。
- 启用鞘翅不同步能力。
# 0.18.0
## 新增
- 引入YACL，但不全，且只作为配置界面。
## 更改
- 更新《模组检查》。
# 0.19.0
## 新增
- 杀戮光环兼容《机械动力武器》的武士刀。
# 0.20.0
## 更改
- 暂时移除了鞘翅自由飞行。
# 0.21.0
## 新增
- 配置现在会保存为文件。
  - 分为全局配置和存档独立配置。
## 更改
- 在Fabric应用鞘翅自由飞行的mixin，在Neoforge不应用。
# 0.22.0
## 更改
- 移除了指令`/trifleclient crit`，由YACL配置界面替代。
# 0.23.0
## 新增
- 配置界面“快速移速”。
## 修复
- 在信雅互联上不被Jade识别插件。
- 在1格宽3格高的无楼梯采矿通道中，快速移动上楼会频繁触发服务端错误移动（[#2](https://github.com/Phoupraw/TrifleClient/issues/2)）
# 0.24.0
## 新增
- 高亮怪物
- 调试“攻击实体”
## 更改
- 暂时移除了鞘翅自由飞行。
# 0.24.1
## 修复
- 在信雅互联上无法暴击。
# 0.24.2
## 修复
- 在信雅互联上无法自动暴击。
# 0.24.3
## 修复
- 在信雅互联上无法自动暴击。
# 0.24.4
## 修复
- 在信雅互联上无法自动暴击。
# 0.24.5
## 修复
- 在信雅互联上，自动暴击可能导致摔死。
# 0.24.6
## 修复
- 以原版机制发光的实体不会发光。（[#5](https://github.com/Phoupraw/TrifleClient/issues/5)）
# 0.24.7
## 修复
- 《虚无世界3》灵巧-跳跃提升会使自动暴击让客户端玩家跳起来。（[#6](https://github.com/Phoupraw/TrifleClient/issues/6))
# 0.25.0
## 更改
- 高亮怪物配置项改为范围。
  - 怪物的高亮颜色现在会渐变。
## 修复
- 高亮怪物距离判断错误。
# 0.26.0
## 新增
- 鱼竿勾到实体时立刻无损收杆并重新抛出。
# 0.26.1
## 修复
- 上版本的新增功能在某些模组鱼钩上不生效。
# 0.26.2
## 修复
- 切竿后不重新抛出。
# 0.27.0
## 新增
- 客户端最小MSPT
# 0.28.0
## 新增
- 客户端最大MSPT
# 0.29.0
## 新增
- 服务器启用飞行。
# 0.29.1
## 修复
- 服务器未启用飞行。
# 0.29.2
## 修复
- 服务器未启用飞行。
# 0.30.0
## 新增
- 自动采摘
- 最小方块速度乘数
# 0.30.1
## 修复
- mixin方法描述不完整
# 0.31.0
## 新增
- 自动收割
# 0.31.1
## 修复
- 依赖版本格式好像不对
# 0.32.0
## 新增
- 自动采摘支持竹子和下界疣
## 修复
- 自动收割并补种由于客户端不同步而导致种子消耗完但仍收割
# 0.32.1
## 修复
- 在自由相机模式下触发自动采摘
# 0.33.0
## 新增
- 自动采摘现在自动兼容农夫乐事的稻米和番茄。
## 更改
- 自动采摘现在放在独立的线程里，防止快速移动时卡顿
## 修复
- 史莱姆和岩浆怪不被高亮
# 0.33.1
## 更改
- 优化自动采摘，减少卡顿
  - 自定义采摘方块暂时无作用
# 0.33.2
## 修复
- 自动收割并发导致的错误
# 0.34.0
## 新增
- 自动收割支持仙人掌（需要手持足够效率的锄头）
- 自动收割支持暮色森林的火炬浆果
- 自动攻击支持暮色森林的巫妖深蓝色法球和九头蛇火球
- 持续举盾
- 按住shift双击左键可以开启持续自动攻击，无需按住左右键
## 更改
- 优化自动收割
## 修复
- 进入存档后需要打开一次配置屏幕才能使存档独立的配置生效
# 0.34.1
## 修复
- 过早访问注册表导致启动时崩溃
# 0.34.2
## 修复
- mixin remap设置错误导致启动时崩溃
# 0.35.0
## 新增
- 自动选中工具
# 0.35.1
## 更改
- 挖掘延迟改用独立计时
  - 原版挖掘延迟现在始终为0
## 修复
- 瞬间破坏无法选中自动选中工具之前的槽位
# 0.35.2
## 更改
- 退出重进存档后，持续自动攻击关闭
## 修复
- 切换存档后挖掘延迟计时器不重置导致无法破坏方块
- 自动选中工具有时在挖掘结束后不选中回去
# 0.36.0
## 新增
- 手持余烬之灯时自动烧毁周围的荆棘
## 更改
- 攻击实体5刻后才能挖掘方块
# 0.36.1
## 修复
- 余烬之灯不自动烧毁荆棘
# 0.36.2
## 修复
- 注册表冻结事件未被调用
# 0.36.3
## 修复
- 注册表冻结事件未被调用
# 0.36.4
## 修复
- 注册表冻结事件在事件被注册之前调用
# 0.37.0
## 新增
- 限制最大移速
- 限制最大跳跃速度
# 0.38.0
## 新增
- 水上漂
- 挖掘储物抽屉不会触发自动切换工具
- 自动采摘巨魔莓
## 更改
- 自动攻击不再攻击命名的末影螨
## 修复
- 不自动收割稻米
# 0.39.0
## 新增
- 限制速度也会限制FOV
- `/trifleclient find blocks <condition>`
- `/trifleclient find samespace`
- 最大聊天文本长度
## 更改
- 按住shift时不再触发切换工具（[#11](https://github.com/Phoupraw/TrifleClient/issues/11)）
- 自动切换工具优先附魔（[#13](https://github.com/Phoupraw/TrifleClient/issues/13)）
## 修复
- 自动收割补种有时消耗两个种子（[#12](https://github.com/Phoupraw/TrifleClient/issues/12)）
# 0.39.1
## 更改
- 移除了鞘翅自由飞行的代码，功能移动至独立模组
## 修复
- 自动收获后有时补种失败
#
## 修复
- 自动选中工具不能关闭 [#15](https://github.com/Phoupraw/TrifleClient/issues/15)
- 有延迟时自动选中工具容易导致挖掘失败